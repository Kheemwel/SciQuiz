...
public class PhysicsTF extends Activity
{
	...
	private void SAVE() {
		if(scr > sp.getInt("phys_tfscore", 0)) {
			editor.putInt("phys_tfscore", scr);
			editor.commit();
		}
	}
		...
		build
			...
			.setPositiveButton("Retry", new DialogInterface.OnClickListener() {
				...
				public void onClick(DialogInterface di, int i) {
					startActivity(new Intent(getApplicationContext(), PhysicsTF.class));
					finish();
				}
			})
