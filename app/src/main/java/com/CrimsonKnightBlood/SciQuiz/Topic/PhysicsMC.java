...
import com.CrimsonKnightBlood.SciQuiz.Quiz.MCPhysicsChoices;
import com.CrimsonKnightBlood.SciQuiz.Quiz.MCAnswers;
public class PhysicsMC extends Activity
{
	...
	private MCPhysicsChoices C = new MCPhysicsChoices();
	private MCAnswers A = new MCAnswers();

	private int QL = Q.PhysicsQuestion.length;
	private int ql = QL + 1;
	private int scr = 0;
	private int ni = 1;
	private int rndm = 0;
	...
	private void SAVE() {
		if(scr > sp.getInt("phys_mcscore", 0)) {
			editor.putInt("phys_mcscore", scr);
			editor.commit();
		}
	}
		...
		build
			...
			.setPositiveButton("Retry", new DialogInterface.OnClickListener() {
				...
				public void onClick(DialogInterface di, int i) {
					startActivity(new Intent(getApplicationContext(), PhysicsMC.class));
					finish();
				}
			})
