...
import com.CrimsonKnightBlood.SciQuiz.Quiz.MCBiologyChoices;
import com.CrimsonKnightBlood.SciQuiz.Quiz.MCAnswers;
public class BiologyMC extends Activity
{
	...
	private MCBiologyChoices C = new MCBiologyChoices();
	private MCAnswers A = new MCAnswers();

	private int QL = Q.BiologyQuestion.length;
	private int ql = QL + 1;
	private int scr = 0;
	private int ni = 1;
	private int rndm = 0;
	...
	private void SAVE() {
		if(scr > sp.getInt("bio_mcscore", 0)) {
			editor.putInt("bio_mcscore", scr);
			editor.commit();
		}
	}
		...
		build
			...
			.setPositiveButton("Retry", new DialogInterface.OnClickListener() {
				...
				public void onClick(DialogInterface di, int i) {
					startActivity(new Intent(getApplicationContext(), BiologyMC.class));
					finish();
				}
			})
