...
public class BiologyTF extends Activity
{
	...
	private void SAVE() {
		if(scr > sp.getInt("bio_tfscore", 0)) {
			editor.putInt("bio_tfscore", scr);
			editor.commit();
		}
	}
		...
		build
			...
			.setPositiveButton("Retry", new DialogInterface.OnClickListener() {
				...
				public void onClick(DialogInterface di, int i) {
					startActivity(new Intent(getApplicationContext(), BiologyTF.class));
					finish();
				}
			})
