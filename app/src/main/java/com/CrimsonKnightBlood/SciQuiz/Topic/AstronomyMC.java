...
public class AstronomyMC extends Activity
{
	...
	private MCAstronomyChoices C = new MCAstronomyChoices();
	private MCAnswers A = new MCAnswers();

	private int QL = Q.AstronomyQuestion.length;
	private int ql = QL + 1;
	private int scr = 0;
	private int ni = 1;
	private int rndm = 0;
	...
	private void SAVE() {
		if (scr > sp.getInt("astro_mcscore", 0)) {
			editor.putInt("astro_mcscore", scr);
			editor.commit();
		}
	}
	...
	private void UpdateQuestion(int num) {
		question.setText(Q.getAstronomyQuestion(num));
